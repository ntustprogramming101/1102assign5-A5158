...
void draw() {
	...
	switch (gameState) {
		...
		for(int i = 0; i < cabbageX.length; i++){
			...
			if(isHit(cabbageX[i],cabbageY[i],SOIL_SIZE,SOIL_SIZE,playerX,playerY,SOIL_SIZE,SOIL_SIZE)==true) { 
    ...
    for(int i = 0; i < clockX.length; i++){
      ...
      // Requirement #3: Use boolean isHit(...) to detect collision
      if(isHit(clockX[i], clockY[i],SOIL_SIZE,SOIL_SIZE,playerX,playerY,SOIL_SIZE,SOIL_SIZE)==true){
        addTime(15);
...
String convertFrameToTimeString(int frames){
    String timer = "";
    float alltime = float(frames) / 60;
    int timeM=floor(alltime/60);
    int timeS=floor(alltime%60);
    timer = nf(timeM, 2)+":"+nf(timeS, 2);
...
void drawCaution(){								// Requirement #6
  ...
  if(getEnemyIndexByRow(playerRow)!=-1){
    image(caution,soldierX[getEnemyIndexByRow(playerRow)],playerY+4*SOIL_SIZE);
  }
